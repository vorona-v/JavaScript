...
function wordsList(str, subStr) {
    ...
    return words.filter(function (word, index, arr) {
        return word.indexOf(subStr) !== -1 && arr.indexOf(word) === index;
    });
}
...
function getLocalDate(date, isSeconds, isISO) {
    ...
    let newHours = ('0' + date.getHours()).slice(-2);
    let newMinutes = ('0' + date.getMinutes()).slice(-2);

    let myNewDate;
    if (isISO === true) {
        myNewDate = `${newYear}-${newMonth}-${newDate}, ${newHours}:${newMinutes}`;
    }
    else {
        myNewDate = `${newDate}.${newMonth}.${newYear}, ${newHours}:${newMinutes}`;
    }

    if (isSeconds === true) {
        let newSeconds = ('0' + date.getSeconds()).slice(-2);
        myNewDate = `${myNewDate}:${newSeconds}`;
    }
...
function getLocalDay(date) {
    ...
    return newDate.getDay() ? newDate.getDay() : 7;
}
...
function getDateAgo(date, days) {
    ...
    return `${newDate.getDate()}.${('0' + (newDate.getMonth() + 1)).slice(-2)}.${newDate.getFullYear()}`;
}
...
let functionality = {
    ...
    set used(value) {
        if (value === 'new' && this.yearNow !== this.year) {
