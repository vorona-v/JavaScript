...
function checkEmail(str) {
    return (/^[a-zA-Z]+\W?[a-zA-z]+@[a-zA-z\.]+\.[a-z]{2,3}$/g).test(str);
}
...
function checkDomainUrl(str) {
    return (/^https?:\/\/(?!:\/\/)([a-zA-Z0-9-_]+\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\.[a-zA-Z]{2,5}?$/g).test(str);
}
...
function createLinksFromDomains(str) {
    let reg = new RegExp('https?:\\/\\/(?!:\\/\\/)([a-zA-Z0-9-_]+\\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\\.[a-zA-Z]{2,5}', 'gi');

    str.match(reg).forEach((item) => str = str.replace(item, `<a href="${item}" target="_blank">${item.replace(/^https?:\/\//gi, '')}</a>`));
